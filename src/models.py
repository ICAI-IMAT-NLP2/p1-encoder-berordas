...
class AttentionHead(nn.Module):
    ...
    def __init__(self, d_model: int, d_k: int, d_q: int, d_v: int):
        ...
        self.wq = nn.Linear(d_model, d_q, bias=True)
        self.wk = nn.Linear(d_model, d_k, bias=True)
        self.wv = nn.Linear(d_model, d_v, bias=True)
...
class FeedForward(nn.Module):
    ...
    def __init__(self, d_model: int, intermediate_size: int):
        ...
        self.linear_1 = nn.Linear(d_model, intermediate_size, bias=True)
        self.linear_2 = nn.Linear(intermediate_size, d_model, bias=True)
        self.gelu = nn.GELU()
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        attn_out = self.attention(y)
        hidden_state = x + attn_out
        
        # Apply layer normalization and then apply feed-forward network
        z = self.layer_norm_2(hidden_state)
        ffn_out = self.feed_forward(z)
        x = hidden_state + ffn_out
